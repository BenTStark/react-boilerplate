...
export class List extends Component {
  constructor(props) {
    ...
    Object.keys(localStorageParameters).map((key) => {
      ...
      if (
        localStorage.getItem(localStorageName) !== undefined &&
          ...
          localStorageItem.map((columnStorage) => {
            localStorageParameters[key].value.map((column, index) => {
              if (columnStorage.dataField === column.dataField) {
                localStorageParameters[key].value[index].hidden =
                  columnStorage.hidden;
              }
            });
          });
        } else {
          localStorageParameters[key].value = JSON.parse(
            localStorage.getItem(localStorageName)
          );
        }
      }
    ...
    this.state = {
      ...
      localStorageNameColumns:
        this.props.endpoint + localStorageParameters.defaultColumns.name,
      selectOptions: localStorageParameters.defaultColumnsVisible.value,
      localStorageNameColumnsVisible:
        this.props.endpoint + localStorageParameters.defaultColumnsVisible.name,
      ...
      localStorageNameSorted:
        this.props.endpoint + localStorageParameters.defaultSorted.name,
      localStorageNamePagination:
        this.props.endpoint + localStorageParameters.defaultPagination.name,
      localStorageNamePage:
        this.props.endpoint + localStorageParameters.defaultPage.name,
    ...
    this.state.selected.map((keyFieldValue) => {
      axios({
        ...
        data: this.state.data
          .filter((item) => _.isEqual(item[this.state.keyField], keyFieldValue))
          .map((item) =>
            _.pick(
              item,
              this.state.columns
                .filter((column) => _.isEqual(column.isPrimaryKey, true))
        ...
        this.setState({
          data: this.state.data.filter(
            (item) => !_.isEqual(item[this.state.keyField], keyFieldValue)
          ),
          selected: this.state.selected.filter(
            (item) => !_.isEqual(item, keyFieldValue)
          ),
        });
      });
    });
    ...
    this.postData(
      _.omit(
        ...
        Object.keys(
          this.state.columns
            .map((column) => {
              return column.autofill ? column.dataField : null;
            })
            .filter((element) => {
              return element != null;
            })
  ...
  checkCSVMetaData = (meta, metaTarget) => {
    ...
    if (!_.isEqual(meta.sort(), metaTarget.sort())) {
      console.log(
        "At least one column name in CSV does not match column name in table"
      );
      return false;
    }
    return true;
  ...
  handleOnDrop = (data) => {
    ...
    this.state.columns.map((column) =>
      column.autofill ? null : metaTarget.push(column.dataField)
    );
  ...
  handleOnSelect = (row, isSelect) => {
    if (isSelect) {
      this.setState({
        selected: [...this.state.selected, row[this.state.keyField]],
      });
    } else {
      this.setState({
        selected: this.state.selected.filter(
          (x) => x !== row[this.state.keyField]
        ),
      });
    }
  };
  ...
  onSort = (field, order) => {
    localStorage.setItem(
      this.state.localStorageNameSorted,
      JSON.stringify([
        {
          dataField: field,
          order: order,
  ...
  selectVisibleColumns = (options, action) => {
    ...
    switch (action.action) {
      case "remove-value":
        columnIndex = columns.findIndex(
          (column) => column.dataField === action.removedValue.value
        );
        columns[columnIndex]["hidden"] = true;
        columnIndex = selectOptions.findIndex(
      ...
      case "select-option":
        ...
        columnIndex = columns.findIndex(
          (column) => column.dataField === action.option.value
        );
    ...
    localStorage.setItem(
      this.state.localStorageNameColumns,
      JSON.stringify(columns)
    );
    localStorage.setItem(
      this.state.localStorageNameColumnsVisible,
  ...
  render() {
    ...
    const paginationOptions = {
      ...
      lastPageText: "Last",
      page:
        Math.ceil(this.state.data.length / this.state.defaultPagination) <
      ...
      onPageChange: (page, sizePerPage) => {
        localStorage.setItem(
          this.state.localStorageNamePage,
          JSON.stringify(page)
        );
      ...
      onSizePerPageChange: (sizePerPage, page) => {
        localStorage.setItem(
          this.state.localStorageNamePagination,
          JSON.stringify(sizePerPage)
        );
            ...
            <div>
              ...
              <Select
                defaultValue={Object.values(
                  _.pickBy(this.state.selectOptions, (value, key) => {
                    return !value.hidden;
                  })
                )}
                isMulti
              ...
              <InputModal
                ...
                columns={this.state.columns.map((column) => {
                  column.value = this.state.edit[column.dataField];
                  return column;
                })}
